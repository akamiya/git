...
static void flush_output(struct rewrite_options *opt)
{
...
static int err(struct rewrite_options *opt, const char *err, ...)
{
	va_list params;
...
static int show(struct rewrite_options *opt, int v)
{
...
static void output(struct rewrite_options *opt, int v, const char *fmt, ...)
{
	va_list ap;
...
static void repo_output_commit_title(struct rewrite_options *opt,
...
static void output_commit_title(struct rewrite_options *opt, struct commit *commit)
{
	repo_output_commit_title(opt, the_repository, commit);
}

static int add_cacheinfo(struct rewrite_options *opt,
...
static inline int merge_detect_rename(struct rewrite_options *opt)
{
...
static int unpack_trees_start(struct rewrite_options *opt,
			      struct tree *common,
			      struct tree *head,
			      struct tree *merge)
{
...
static void unpack_trees_finish(struct rewrite_options *opt)
{
			   ...
			   unsigned int mode, void *context)
{
	...
	struct rewrite_options *opt = context;
...
static void get_files_dirs(struct rewrite_options *opt, struct tree *tree)
{
...
static void record_df_conflict_files(struct rewrite_options *opt,
				     struct string_list *entries)
{
...
static int update_stages(struct rewrite_options *opt, const char *path,
...
static int remove_file(struct rewrite_options *opt, int clean,
...
static char *unique_path(struct rewrite_options *opt,
			 const char *path,
			 const char *branch)
{
	struct path_hashmap_entry *entry;
 ...
 */
static int was_tracked_and_matches(struct rewrite_options *opt, const char *path,
				   const struct diff_filespec *blob)
{
 ...
 * Returns whether path was tracked in the index before the merge started
 */
static int was_tracked(struct rewrite_options *opt, const char *path)
{
	int pos = index_name_pos(&opt->priv->orig_index, path, strlen(path));
...
static int would_lose_untracked(struct rewrite_options *opt, const char *path)
{
	struct index_state *istate = opt->repo->index;
...
static int was_dirty(struct rewrite_options *opt, const char *path)
{
	struct cache_entry *ce;
	int dirty = 1;
...
static int make_room_for_path(struct rewrite_options *opt, const char *path)
{
	int status, i;
...
static int update_file_flags(struct rewrite_options *opt,
...
static int update_file(struct rewrite_options *opt,
		       int clean,
		       const struct diff_filespec *contents,
		       const char *path)
{
...
static int merge_3way(struct rewrite_options *opt,
		      ...
		      const int extra_marker_size)
{
	...
	if (opt->priv->call_depth) {
		...
		case REWRITE_VARIANT_OURS:
			ll_opts.variant = XDL_MERGE_FAVOR_OURS;
			break;
		case REWRITE_VARIANT_THEIRS:
...
static int merge_submodule(struct rewrite_options *opt,
...
static int merge_mode_and_contents(struct rewrite_options *opt,
				   const struct diff_filespec *o,
				   const struct diff_filespec *a,
				   const struct diff_filespec *b,
				   const char *filename,
...
static int handle_rename_via_dir(struct rewrite_options *opt,
				 struct rename_conflict_info *ci)
{
	/*
	...
	int mark_conflicted = (opt->detect_directory_renames ==
			       REWRITE_DIRECTORY_RENAMES_CONFLICT);
...
static int handle_change_delete(struct rewrite_options *opt,
...
static int handle_rename_delete(struct rewrite_options *opt,
				struct rename_conflict_info *ci)
{
	const struct rename *ren = ci->ren1;
...
static int handle_file_collision(struct rewrite_options *opt,
...
static int handle_rename_add(struct rewrite_options *opt,
			     struct rename_conflict_info *ci)
{
...
static char *find_path_for_conflict(struct rewrite_options *opt,
				    const char *path,
				    const char *branch1,
				    const char *branch2)
{
...
static int handle_rename_rename_1to2(struct rewrite_options *opt,
				     struct rename_conflict_info *ci)
{
...
static int handle_rename_rename_2to1(struct rewrite_options *opt,
				     struct rename_conflict_info *ci)
{
 ...
 */
static struct diff_queue_struct *get_diffpairs(struct rewrite_options *opt,
					       struct tree *o_tree,
					       struct tree *tree)
{
 ...
 */
static char *handle_path_level_conflicts(struct rewrite_options *opt,
					 const char *path,
					 struct dir_rename_entry *entry,
 ...
 */
static void handle_directory_level_conflicts(struct rewrite_options *opt,
...
static char *check_for_directory_rename(struct rewrite_options *opt,
					const char *path,
					struct tree *tree,
					struct hashmap *dir_renames,
...
static void apply_directory_rename_modifications(struct rewrite_options *opt,
						 struct diff_filepair *pair,
						 char *new_path,
						 struct rename *re,
						 struct tree *tree,
 ...
 */
static struct string_list *get_renames(struct rewrite_options *opt,
				       const char *branch,
				       struct diff_queue_struct *pairs,
				       struct hashmap *dir_renames,
				       struct hashmap *dir_rename_exclusions,
...
static int process_renames(struct rewrite_options *opt,
...
static int detect_and_process_renames(struct rewrite_options *opt,
				      struct tree *common,
				      struct tree *head,
				      struct tree *merge,
				      struct string_list *entries,
				      struct rename_info *ri)
{
	...
	if ((opt->detect_directory_renames == REWRITE_DIRECTORY_RENAMES_TRUE) ||
	    (opt->detect_directory_renames == REWRITE_DIRECTORY_RENAMES_CONFLICT &&
	     !opt->priv->call_depth)) {
...
static int read_oid_strbuf(struct rewrite_options *opt,
...
static int blob_unchanged(struct rewrite_options *opt,
			  const struct diff_filespec *o,
			  const struct diff_filespec *a,
...
static int handle_modify_delete(struct rewrite_options *opt,
				const char *path,
				const struct diff_filespec *o,
				const struct diff_filespec *a,
				const struct diff_filespec *b)
...
static int handle_rename_normal(struct rewrite_options *opt,
				const char *path,
				const struct diff_filespec *o,
				const struct diff_filespec *a,
				const struct diff_filespec *b,
...
static int warn_about_dir_renamed_entries(struct rewrite_options *opt,
					  struct rename *ren)
{
	const char *msg;
	int clean = 1, is_add;
	...
	/* Sanity checks */
	assert(opt->detect_directory_renames > REWRITE_DIRECTORY_RENAMES_NONE);
	assert(ren->dir_rename_original_type == 'A' ||
	       ren->dir_rename_original_type == 'R');

	/* Check whether to treat directory renames as a conflict */
	clean = (opt->detect_directory_renames == REWRITE_DIRECTORY_RENAMES_TRUE);
...
static int process_entry(struct rewrite_options *opt,
...
static int rewrite_trees_internal(struct rewrite_options *opt,
				struct tree *head,
				struct tree *merge,
				...
				struct tree **result)
{
	...
	if (unmerged_index(istate)) {
		 ...
		 * through the entries. Keeping it in the rewrite_options as
 ...
 */
static int rewrite_recursive_internal(struct rewrite_options *opt,
				    struct commit *h1,
				    struct commit *h2,
				    ...
				    struct commit **result)
{
	...
	for (iter = merge_bases; iter; iter = iter->next) {
		...
		if (rewrite_recursive_internal(opt, merged_merge_bases, iter->item,
	...
	clean = rewrite_trees_internal(opt,
...
static int merge_start(struct rewrite_options *opt, struct tree *head)
{
	struct strbuf sb = STRBUF_INIT;
	...
	assert(opt->detect_directory_renames >= REWRITE_DIRECTORY_RENAMES_NONE &&
	       opt->detect_directory_renames <= REWRITE_DIRECTORY_RENAMES_TRUE);
	assert(opt->rename_limit >= -1);
	assert(opt->rename_score >= 0 && opt->rename_score <= MAX_SCORE);
	assert(opt->show_rename_progress >= 0 && opt->show_rename_progress <= 1);

	assert(opt->xdl_opts >= 0);
	assert(opt->recursive_variant >= REWRITE_VARIANT_NORMAL &&
	       opt->recursive_variant <= REWRITE_VARIANT_THEIRS);
...
static void merge_finalize(struct rewrite_options *opt)
{
	flush_output(opt);
		...
		struct tree *merge_base)
{
	...
	if (merge_start(opt, head))
		return -1;
	clean = rewrite_trees_internal(opt, head, merge, merge_base, &ignored);
	merge_finalize(opt);
		    ...
		    struct commit **result)
{
	...
	if (merge_start(opt, repo_get_commit_tree(opt->repo, h1)))
		return -1;
	clean = rewrite_recursive_internal(opt, h1, h2, merge_bases, result);
	merge_finalize(opt);
			    ...
			    struct commit **result)
{
	...
	clean = rewrite_recursive(opt, head_commit, next_commit, ca,
				result);
	if (clean < 0) {
...
static void merge_recursive_config(struct rewrite_options *opt)
{
	char *value = NULL;
	int renormalize = 0;
	...
	if (!git_config_get_string("merge.directoryrenames", &value)) {
		...
		if (0 <= boolval) {
			opt->detect_directory_renames = boolval ?
				REWRITE_DIRECTORY_RENAMES_TRUE :
				REWRITE_DIRECTORY_RENAMES_NONE;
...
void init_rewrite_options(struct rewrite_options *opt,
			struct repository *repo)
{
	const char *merge_verbosity;
	memset(opt, 0, sizeof(struct rewrite_options));
...
/*
 * For now, members of rewrite_options do not need deep copying, but
 * it may change in the future, in which case we would need to update
 * this, and also make a matching change to clear_rewrite_options() to
...
void clear_rewrite_options(struct rewrite_options *opt UNUSED)
{
...
int parse_rewrite_opt(struct rewrite_options *opt, const char *s)
{
	...
	if (!s || !*s)
		return -1;
	if (!strcmp(s, "ours"))
		opt->recursive_variant = REWRITE_VARIANT_OURS;
